...
export default class DetailPage extends Component {
    ...
    renderActivities() {
        ...
        if (!activities || !activities.length) {
            ...
            return ( <
                Animated.View style = {
                    [styles.actives, { opacity: this.state.activeOpacity }] } > {
                    activities.map((item, i) => {
                        return ( <
                            View key = { i }
                            style = {
                                { flexDirection: "row", alignItems: "center", height: px2dp(18) } } >
                            <
                            Text style = {
                                { fontSize: px2dp(11), color: "#fff", backgroundColor: color[item.key] || "#f1884f", paddingHorizontal: 1, paddingVertical: 1 } } > { item.key } < /Text> <
                            Text numberOfLines = { 1 }
                            style = {
                                { fontSize: px2dp(11), marginLeft: 3, color: "#fff" } } > { item.text } < /Text> <
                            /View>
                        )
                    })
                } <
    ...
    render() {
            ...
            return ( <
                    ...
                    <
                    Animated.Image source = { LocalImg.bg }
                    ref = { 'backgroundImage' }
                    ...
                    <
                    Animated.View style = {
                        { flexDirection: "row", paddingHorizontal: 16, opacity: this.state.headOpacity } } >
                    <
                    Image source = { LocalImg.bg }
                    style = { styles.logo }
                    /> <
                    View style = {
                    ...
                    <
                    View style = {
                        { flexDirection: "row", paddingTop: 8, paddingBottom: 18 } } > {
                        data.fengniao ? ( < Text style = {
                                [styles.label2, { marginRight: 5 }] } > { "蜂鸟专送" } < /Text>):null} <
                            Text style = {
                                { color: "#fff", fontSize: px2dp(12) } } > { `${data.time}分钟送达` } < /Text> <
                            /View> <
                            /TouchableOpacity> <
                            Text style = {
                                { color: "#fff", fontSize: px2dp(12) } }
                            numberOfLines = { 1 } > { data.bulletin } < /Text> <
                            /View> <
                            /Animated.View> { this.renderActivities() } <
                            /View> { this.renderGoods() } <
                            NavBar title = { data.name }
                            titleStyle = {
                                { opacity: this.state.titleOpacity } }
                            style = {
                                { backgroundColor: "transparent", position: "absolute", top: 0, width } }
                            ...
                            rightPress = {
                                () => {} }
                            /> <
                            Parabolic ref = { "parabolic" }
                            style = {
                                [styles.tmpBtn, { top: this.state.addBtnY }] }
                            renderChildren = {
                                () => {
                                    return ( <
                                        View style = {
                                            { width: px2dp(14), height: px2dp(14), backgroundColor: "#3190e8", borderRadius: px2dp(7), overflow: "hidden" } } > < /View>
                                    )
                                }
                            }
